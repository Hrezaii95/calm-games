...
btn.focus(); // Give initial focus to start button
...
restart.setAttribute("tabindex", "0"); // Make restart button focusable
...
// NEW: Add Enter key support for the button
...
// Add Enter key support for restart button
 ...
 *or check if enter pressed, or spacebar to skip*/
inWord.addEventListener("keyup", (e) => {
  console.log(e);
  // Only trigger winChecker if input field is visible and enabled
  if (inWord.style.display !== "none" && !inWord.disabled) {
    if (inWord.value.length === game.sel.length) {
      // Full word typed - check if correct
      winChecker();
    } else if (e.key === "Enter" && inWord.value.length > 0) {
      // Enter pressed with some text - submit as guess
      winChecker();
    } else if (e.key === " " || e.key === "Spacebar") {
      // Skip word - clear input and trigger as incorrect
      inWord.value = "";
      winChecker();
    }
    // If Enter pressed with no text, do nothing
  }
});
...
function gamePlay() {
  if (myWordsFromJson.length <= 0 || maxGuesses === 5) {
    ...
    restart.focus(); // Give focus to play again button
...
function winChecker() {
  if (inWord.value.toLowerCase() == game.sel) {
    ...
    btn.focus(); // NEW: Give focus to the button so Enter works
    restart.style.display = "block";
